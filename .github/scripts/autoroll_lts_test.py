...
class TestChromiumVerification(unittest.TestCase):
  ...
  def test_chromium_cherry_pick_assertion_error(
    ...
    """Test chromium_cherry_pick raises RuntimeError if verification fails."""
    mock_git.return_value = 'fake_sha\n'
    with patch('sys.stderr'):
      with self.assertRaises(RuntimeError):
        autoroll_lts.chromium_cherry_pick(
            ...
            ('d', 'a', 'm'),
            True,
            ('.github/AUTOROLL_CHROMIUM', 'sha123'),
        )
...
class TestChromiumCherryPick(unittest.TestCase):
  ...
  @patch('autoroll_lts.verify_chromium_commit', return_value=True)
  @patch('autoroll_lts.remove_local_checkout')
  @patch('autoroll_lts.replace_submodules_with_dirs')
  @patch('autoroll_lts.git')
  @patch('autoroll_lts.apply_and_commit')
  def test_chromium_cherry_pick(self, mock_apply_and_commit, mock_git,
    ...
    mock_git.side_effect = lambda *args, **kwargs: 'revert_sha' if args[
        0] == 'rev-parse' else ''
    mock_apply_and_commit.return_value = ('status', 'unmerged')
