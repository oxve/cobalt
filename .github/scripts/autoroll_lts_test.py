...
class TestChromiumCherryPick(unittest.TestCase):
  ...
  @patch('autoroll_lts.get_submodule_root_dirs')
  ...
  def test_chromium_cherry_pick(self, mock_revert, mock_git, mock_remove,
                                mock_rmtree, mock_exists, mock_isdir,
                                mock_islink, mock_roots):
    mock_roots.return_value = ['link1', 'dir2', 'file3']
    mock_islink.side_effect = lambda x: x == 'link1'
    mock_isdir.side_effect = lambda x: x == 'dir2'
    mock_exists.side_effect = lambda x: x in ('link1', 'dir2', 'file3')
    ...
    mock_rmtree.assert_called_once_with('dir2', ignore_errors=True)
    self.assertEqual(mock_remove.call_count, 2)
    mock_remove.assert_any_call('link1')
    mock_remove.assert_any_call('file3')
    mock_git.assert_any_call('rm', '-rf', '--', '.')
    mock_git.assert_any_call('clean', '-ffdx')
